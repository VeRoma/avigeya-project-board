...
function renderTaskCard(task, isUserView, statuses) {
    ...
    return `<div class="task-container" draggable="true" data-task-id="${task.id}" data-status-group="${task.status.name}">
                <div class="task-header flex justify-between items-center gap-3 cursor-pointer select-none">
                    ...
                    <div class="task-status-checker status-action-area" data-status="${task.status.name}">
                        ${statusIcon}
                    </div>
                </div>
                <div id="task-details-${task.id}" class="task-details collapsible-content" data-version="${task.version}" data-task='${taskDataString}'></div>
            </div>`;
}
...
export function renderProjects(projects, userName, userRole, expandedState = {}, filters = {}) {
    ...
    projects.forEach(project => {
        ...
        // Сравниваем по имени, так как в справочнике allProjects поле называется 'name'.
        const fullProject = allProjects.find(p => p.name === project.name);

        if (!fullProject) return; // Пропускаем, если проект не найден
        
        // --- ИСПРАВЛЕНИЕ: Отображаем проекты, даже если в них нет задач ---
        // Закомментируем эту строку, чтобы пустые проекты тоже были видны.
        ...
        const manageMembersButton = isAdminOrOwner
            ? `<button class="manage-members-btn p-2 rounded-full absolute top-2 right-12 hover:bg-gray-200 dark:hover:bg-gray-700 z-10" data-project-id="${fullProject.id}" data-project-name="${fullProject.name}" title="Управлять участниками">
                  <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
              </button>`
            : '';

        const manageStagesButton = isAdminOrOwner
            ? `<button class="manage-stages-btn p-2 rounded-full absolute top-2 right-2 hover:bg-gray-200 dark:hover:bg-gray-700 z-10" data-project-id="${fullProject.id}" data-project-name="${fullProject.name}" title="Настроить этапы">
